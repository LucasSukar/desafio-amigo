angular.module("amigoApp").directive("amigoFeedCard", function () {
  return {
    ...
    template: [
      ...
      '      <button class="feed-card__menu-btn" ng-click="toggleOpcoes()"><i class="fa-solid fa-ellipsis-vertical"></i></button>',
      '      <div class="feed-card__menu-panel" ng-show="post.mostrarOpcoes">',
      '        <button class="feed-card__menu-action" ng-if="post.allowEdit" ng-click="editar()"><i class="fa-solid fa-pen"></i> Editar</button>',
      '        <button class="feed-card__menu-action feed-card__menu-action--danger" ng-if="post.allowRemove" ng-click="deletar()"><i class="fa-solid fa-trash"></i> Apagar</button>',
      "      </div>",
      "    </div>",
      "  </div>",
