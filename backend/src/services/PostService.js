...
function formatPost(postJSON, meId, forceOwner = false) {
  return {
    ...
    jaCurtiu: meId
      ? postJSON.likes.some((l) => l.user_id == meId)
      : false,
    likes: undefined,
  };
}
...
class PostService {
  ...
  async findById(id, meId) {
    ...
    return formatPost(postJSON, meId);
  }

  async create({ title, content, resume, data_publicacao }, meId) {
    return Post.create({ title, content, resume, data_publicacao, user_id: meId });
  }
  ...
  async update(id, meId, { title, content, resume }) {
    ...
    if (post.user_id != meId) {
      throw { status: 401, message: POST_MESSAGES.NO_PERMISSION_EDIT };
    }
  ...
  async delete(id, meId) {
    ...
    if (post.user_id != meId) {
      throw { status: 401, message: POST_MESSAGES.NO_PERMISSION_DELETE };
    }
